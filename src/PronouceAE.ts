...
export function indentifyWordSegments(word: AnnotatedLetter[]): [AnnotatedLetter[]] {
  ...
  for (var i = 0; i < word.length; i++) {
    ...
    if ( ( word[i].plainText === "'" && word[i+1].plainText !== 'r' ) ||
      word[i].annotations.some( (annotation) => annotation.includes("seperator") ) || 
      ( word[i].plainText === "-" ) ) {
    ...
    } else if ( ( word[i].plainText === "'" )) { // remove " ' "
      word.splice(i,1)
      i--
    }
  }
...
export function addVowelAndConsonantMarkers(segment: AnnotatedLetter[]): void {
  for (var i = 0; i < segment.length; i++) {
    ...
    } else {//if (isConsonant(segment, i)) {
      segment[i].groupType = "consonant"
    // } else {
    //   console.log("not vowel or consonant ?")
    }
  }
}
...
export function makeVowelDigraphs(segment: AnnotatedLetter[]): void {
  for (var i = 1; i < segment.length; i++) {
    if (isUnAnnotatedVowel(segment[i]) && isUnAnnotatedVowel(segment[i-1])) {
      if (digraphs.vowel.includes(segment[i-1].plainText+segment[i].plainText)) {
...
export function makeConsonantDigraphs(segment: AnnotatedLetter[]): void {
  digraphs.consonant.forEach(currentDigraph => {
    for (var i = 1; i < segment.length; i++) {
      if (isUnAnnotatedConsonant(segment[i]) && isUnAnnotatedConsonant(segment[i-1])) {
        if (currentDigraph === (segment[i-1].plainText+segment[i].plainText)) {
          segment[i-1].plainText += segment[i].plainText
          segment[i-1].digraph = true
          segment.splice(i,1)
        }
