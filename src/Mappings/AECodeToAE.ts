...
function getAnnotatedText(letter: AnnotatedLetter): void {
  let marks = letter.annotations.map(mark => annotations[mark].unicode).join("")
  if (!letter.digraph) {  
...
export function AnnotatedTextToAnnotatedCode(word: string): AnnotatedLetter[] {
  ...
  for (var i = 0; i < marks.length; i++) {
    ...
    wordCode.push(makeAnnotatedLetter(plainText[i], marks[i], digraph, "undetermined"))
  }
  return wordCode
}
...
function getAnnotationsAsArray(word: string): [annotation[]] {
  ...
  for (var i = 0; i < word.length; i++) { // OTHER DOUBLE ANNOTATIONS
    if (word.charCodeAt(i) === 8203) {
      mark = ["digraph", ...getAnnotationsFrom(word, i+1)]
    } else if ( i + 1 == word.length || !annotationForUnicode(word.charCodeAt(i+1)) ) {
      mark = []
    } else {
...
function getPlainTextAsArray(word: string): string[] {
  ...
  for (var i = 0; i < word.length; i++) {
    if (annotationForUnicode(word.charCodeAt(i)) && word.charCodeAt(i) !== 8203) {
